...
classdef flickering < handle
    ...
    methods
        ...
        function obj=flickering(filename,Nrad,dpix,Temp,varargin)
            ...
            elseif strfind(filename,'.tiff')            % this in the case you have tiffs
                moviename = filename(1:end-12);
                obj.video = TiffsReader(moviename);
                pics = 'tiffs';
            else
                obj.video=VideoReader(filename);
            end
            %load first frame
            obj.frame=get_frame(obj.video,1);               % use this function if you have tiffs 
...
%             end
            ...
            if fluo
%                 ga=fspecial('gaussian',round(50),round(20));      
                gas=fspecial('gaussian',3,1);
%                 bckg=imfilter(obj.frame,ga,'replicate');
%                 obj.frame=imfilter(obj.frame-bckg+max(obj.frame(:)),gas,'replicate');
%                  [gx,gy]=gradient(obj.frame);
%                  obj.frame=imfilter(10+gx.^2+gy.^2,gas,'replicate');
%                  obj.frame=255*(obj.frame-min(obj.frame(:)))./(max(obj.frame(:))-min(obj.frame(:)));
                 obj.frame=imfilter(obj.frame,gas,'replicate');
                 %obj.frame=medfilt2(obj.frame,[3 3]);
            else
                gas=[];
            end
            ...
            ang=linspace(0,2*pi-2*pi/obj.Nrad,obj.Nrad);
...
%                 for hh = 0:10                   % this is if you have corrupted frames 
%                     cc = cen(range(nn-hh),:);
%                     if all(cc == zeros(1,2))==0
%                         moving_center = cc;
%                         break
%                     else
%                         continue
...
%                 end
                ...
                circleinfo.center = cen(range(nn),:);
                ...
                try
                    parfor k=range(nn):range(nn+1);
                        ...
                        %center = moving_center;                 % when you have corrupted frames
                        center=circleinfo.center;
                        method = 'profile';
                        
                        if all(isnan(ffs(:,:,k-range(nn)+1)))==0
                        ...
                        if fluo      
                            gas=fspecial('gaussian',3,1);
                            frame=imfilter(frame,gas,'replicate');   
                        else
                            gas=[];
                        end
                        ...
                        contour_fine(k,:)=radius_limits(1)-fit_contour(rc{k})-floor(length(template)/4)-offset_template;
        ...
        function obj=analyse_contour_raw(obj,varargin)
            ...
            if fluo
                ga=fspecial('gaussian',round(obj.video.height/12),round(obj.video.height/150));
                bckg=imfilter(obj.frame,ga,'replicate');
            ...
            for nn=1:NF;
                ...
                obj.frame = flat_field_correction(obj.frame,polVal);            % correction for the background
        ...
        function obj=get_spectrum(obj,varargin)
            ...
            plotting=false;
            frame_range=2:size(obj.contour_fine,1);
            cc=obj.contour_fine;
            if nargin==2
                plotting=varargin{1};
            elseif nargin==3
            ...
            rr=rr-repmat(mean(rr,2),1,size(rr,2));    %??? why not?
            ...
            data.static.range=data.static.q>data.static.q(4)&data.static.q<data.static.q(13);
            ...
            if plotting
                ...
                %clf
                hold on
                errorbar(data.static.q(2:end),data.static.ps(2:end),data.static.err(2:end),'bo')
...
%                 hold on
%                 %clf
%                 errorbar(data.dynamic.omega(3:end),data.dynamic.ps(3:end),data.dynamic.err(3:end),'bo')
%                 
%                 xlabel('\omega (rad/s)')
%                 ylabel('PSD (m^2/s)')
                ...
                hold on
                %clf
                plot(data.corr.q,data.corr.tau,'bo')
                set(gca,'yscale','log','xscale','log')
                xlabel('q (m^{-1})')
                ylabel('\tau (s)')
        ...
        function obj=fit_vesicles_fluctuations(obj,varargin)
            ...
            data.static.range=6:20;
            rr=data.static.range;
            %err=data.static.err(rr)./sqrt(obj.dt*size(obj.contour_fine,1)./data.corr.tau(rr-1));
            %Davide
...
%                 modelcorr=tauq(log10(data.eta),log10(data.s),log10(data.k),data.gamma,data.R,qt);
                ...
                plot(data.static.q(3:end),data.static.ps(3:end),'r.')
                hold on
...
%                 drawnow
    ...
    methods (Hidden)
        ...
        function obj=update_info(obj,ring,contour)
            obj.ring=ring;
            ang=linspace(0,2*pi-2*pi/length(contour),length(contour));
...
%membrane
ang=linspace(0,2*pi-2*pi/Nrad,Nrad);
...
elseif strcmp(method,'profile')
    %mask=make_mask(r1);                % don't use any mask: done this to
    %automize the analysis
    ...
    %t = mean(rrsh,2);
    %[YTS,XTS] = max(t);
    [XTS,YTS]=ginput(1)                    % don't use the manual selection: done this to
    %automize the analysis
    XTS=round(XTS);
...
contour=radius_limits(1)-p-L/4;
contour=clean_cont(contour,5,size(ringcorr,1));
